...
#
## Script executes prepare_gtex_ase.py to generates ASE table for given tissue.
#

# By default, this script submit jobs to the cluster workload manager SLURM,
# but it's also possible to run the script on local machine.

set -Eeu -o pipefail

# Workingspace
projdir=$(readlink -f ~/Documents/projects/wp_ase_dlp)
...
lookup_table=$iptdir/GTEx/sample/GTEx_Analysis_2017-06-05_v8_WholeGenomeSeq_838Indiv_Analysis_Freeze.lookup_table.txt

# Output
mkdir -p $optdir/allelic_read_counts/GTEx/$tissue
...
if [[ $use_cluster == true ]]; then
    ...
    logfile=$optdir/allelic_read_counts/GTEx/$tissue/%j-%u-prepare_gtex_ase.log
...
for ase_table in $ase_table_path/*.tsv.gz; do
    ...
    if [[ -f $optdir/allelic_read_counts/GTEx/$tissue/\$output_file ]]; then
    ...
    if [[ \$has_tissue -gt 0 ]]; then
        ...
        python $scpdir/misc/prepare_gtex_ase.py \
            ...
            -a \$ase_table \
            -t $tissue \
            -o $optdir/allelic_read_counts/GTEx/$tissue/\$output_file &
    ...
    n_running_jobs=\$(jobs | grep -wc Running)
