...
class Trainer:
    ...
    def _test(self, model_state: str = None, testloader: DataLoader = None, criterion=None):
        ...
        with torch.no_grad():
            for data in testloader:
                ...
                if device == 'cpu':
                    inputs_type = torch.FloatTensor
                else:
                    inputs_type = torch.cuda.FloatTensor

                inputs, true_label = data[0].type(inputs_type).to(device), data[1].to(device)
        ...
        for cv_idx, (train_idx, test_idx) in enumerate(splits):
            ...
            for epoch in range(eps):
                ...
                if (epoch + 1) % log_per_n_epoch == 0:
                    ...
                    logging.info('{: ^2} | Train  | {: ^5} | {: ^6.2f} | {: ^6.2f} | {: ^6.2f} | {: ^6.2f} | {:.4f}'.format(cv_idx + 1, epoch+1, tn_accu, tn_prec, tn_rcll, tn_roc_auc, running_loss))
                    ...
                    logging.info('{: ^2} | Test   | {: ^5} | {: ^6.2f} | {: ^6.2f} | {: ^6.2f} | {: ^6.2f} | {:.4f}'.format(cv_idx+1, epoch+1, tt_accu, tt_prec, tt_rcll, tt_roc_auc, tt_loss))
    ...
    def train(self, eps=20, criterion=None, optimizer=None, learning_rate=1e-5, **kwargs):
        ...
        self._train(eps, optimizer, device, criterion, splits, batch_size, shuffle, num_workers, log_per_n_epoch=log_per_n_epoch)
