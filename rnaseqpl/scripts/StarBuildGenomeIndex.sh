...
opt=$(getopt -l "workDir:,starGenomeIndexDir:,genomeFastaFile:,genomeAnnotationsFile:,runThreadN:,help" -- "w:f:g:a:" $@)
eval set -- ${opt}
while true; do
	case $1 in
...
starGenomeIndexDir=${workDir}/${starGenomeIndexDir:?-g/--starGenomeIndexDir is required}
