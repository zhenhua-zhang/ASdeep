...
两者可以位于基因上游、下游以及内含子中，可以距离基因远达1Mnp（1,000,000bp）。
我们认为调控元件中杂合位点的存在会导致等位基因差异表达。
但常见的元件如增强子和沉默子距离基因的位置可能很远，例如1Mbp。
所以我们需要消除非杂合位点的序列在构建模型中的”预测效应“。
目前的想法是用随机数（例如来在标准正态分布）掩盖非杂合位点，保留杂合位点及其两个侧翼特定长度的位点。与此同时，需要把序列扩展到基因上下游各1Mbp的位置，并包含内含子序列，其中内含子可暂时不考虑。
而每个基因的调控元件存在差异，因此我们需要根据已知数据，即调控某基因的增强子和沉默子的位置，来确定实际的输入序列长度。
这个操作只针对确定发生ASE的基因。
如果使用1Mbp，不太容易见Hilbert curve，而考虑到cis-eQTL的位置多在基因上下游500Kbp。
如果使用这个长度，则两个等为基因加起来约等于1Mbp，而级数为10的Hilbert curve的长度很接近1Mbp。
所以可以使用500Kbp作为模型的输入序列。

## 模型的性能太低

在使用500Kbp作为模型的输入序列的情况下，用BIOS的数据训练的模型表现一般。
一个可能的原因是输入序列中可学习的features太少，即杂合位点的数量太少。
表现为在预测的结果中，热图仅有很小的区域未被掩码处理。
这应该是由于BIOS数据中SNP密度太低的原因。
为了确定这个结论，应该去检查BIOS数据中每个候选ASE基因上游500Kbp序列中SNP的数量及分布。
同时也应该检查Geuvadis和GTEx的数据。
如果BIOS数据中SNP密度明显太低，那就不要再用BIOS的数据了。
